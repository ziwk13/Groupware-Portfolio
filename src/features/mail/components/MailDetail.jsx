...
// material-ui
import {Box, Grid, CircularProgress, Button, Table, TableBody, TableRow, TableCell, Alert } from '@mui/material';
...
export default function MailDetail() {
	...
	const myboxMail = mail && mail.mailboxType === "MYBOX" ? true : false;
	const trashMail = mail && mail.mailboxType === "TRASH" ? true : false;
	const myMail = mail && mail.senderEmail === myInfo.email ? true : false;
			...
			<MainCard sx={{minHeight:'100%'}}>
				<Grid container spacing={gridSpacing}>
					...
					<Grid size={12} sx={{display:'flex', gap:'5px'}}>
						{myMail && <Button variant="contained" onClick={() => navigate(`/mail/write/${mailId}`)}>재작성</Button>}
						{canReply && <Button variant="contained" onClick={() => navigate(`/mail/write/${mailId}?mode=reply`)}>회신</Button>}
						{!myboxMail && <Button variant="contained" onClick={() => handleMoveMailType('MYBOX')}>이동</Button>}
						{!trashMail && <Button variant="contained" onClick={() => handleMoveMailType('TRASH')}>삭제</Button>}
						{trashMail && <Button variant="contained" onClick={() => handleDelete()}>영구삭제</Button>}
						...
						<Button variant="contained" onClick={() => navigate(`/mail/list/${mail.mailboxType}`)}>목록</Button>
					</Grid>
									...
									<TableCell colSpan={3}>
										<Chip
											...
											avatar={
												<Avatar
													alt={mail.senderName}
													src={mail.senderProfileImg ? getImageUrl(mail.senderProfileImg) : DefaultAvatar}
												/>
											}
											variant="outlined"
										/>
